...
void epoch_st(const unsigned int pop_size,
             int fitness,
...
{
    unsigned int fitness = calculate_total_fitness(genomes);
    ...
    genome* baby = &babies[0];

    for (unsigned int i = NUM_ELITE * NUM_COPIES_ELITE; i < pop_size; i += 2)
    {
            futures.push_back(std::async (epoch_st, pop_size, fitness,  std::ref(genomes), baby,i));
    }
    for (auto &f : futures)
    {
        f.get();
